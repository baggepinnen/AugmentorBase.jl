@test_broken false # This needs to be re-implemented
#
...
#     for pl in (AugmentorBase.ImmutablePipeline(Rotate90()), (Rotate90(),))
#         img = @inferred AugmentorBase._augment(rect, pl)
...
# end
#
# ops = AugmentorBase.ImmutablePipeline(Rotate(90),Rotate(-90)) # forces affine
# @testset "$(str_showcompact(ops))" begin
#     wv = @inferred AugmentorBase._augment(camera, ops)
#     @test typeof(wv) === typeof(invwarpedview(camera,
#                                     AugmentorBase.toaffinemap(NoOp(),rect), Flat()))
#     @test wv == camera
#     wv1, wv2 = @inferred AugmentorBase._augment((rgb_rect,camera), ops)
...
# @testset "$(str_showcompact(ops))" begin
#     img = @inferred AugmentorBase._augment(camera, ops)
#     @test img === camera
#     img1, img2 = @inferred AugmentorBase._augment((camera, square2), ops)
#     @test img1 === camera
#     @test img2 === square2
# end
#
...
#     img1, img2 = @inferred AugmentorBase._augment((camera, N0f8.(camera)), ops)
...
# @testset "$(str_showcompact(ops))" begin
#     wv = @inferred AugmentorBase._augment(rect, ops)
...
# @testset "$(str_showcompact(ops))" begin
#     img = @inferred AugmentorBase._augment(rect, ops)
#     @test typeof(img) <: OffsetArray
...
#     # however this works:
#     ops2 = (Rotate180(1),Crop(5:200,200:500),Rotate90(1),Crop(1:250, 1:150))
#     wv2 = @inferred AugmentorBase.augment(camera, ops2)
...
# end
#
# ops = AugmentorBase.ImmutablePipeline(Rotate180(), Crop(5:200,200:500), Rotate90(),
#         Crop(50:300, 50:195))
# @testset "$(str_showcompact(ops))" begin
#     wv = @inferred AugmentorBase._augment(camera, ops)
#     @test typeof(wv) <: SubArray
...
#
#     # see above
#     #img = @inferred AugmentorBase.augment(camera, ops)
#     img = AugmentorBase.augment(camera, ops)
#
#     @test img == parent(copy(wv))
#     @test typeof(img) <: Array
#     @test eltype(img) <: eltype(camera)
...
#     wv2 = AugmentorBase._augment(camera, (ops..., NoOp()))
...
#         SplitChannels())
# @testset "$(str_showcompact(ops))" begin
#     wv1 = @inferred AugmentorBase._augment(camera, ops[1:3])
#     wv2 = @inferred AugmentorBase._augment(camera, ops[1:4])
#     wv3 = @inferred AugmentorBase._augment(camera, ops)
#
#     _, wv4 = @inferred AugmentorBase._augment((rotl90(camera), camera), ops)
#     @test wv3 === wv4
...
# @test typeof(@inferred(AugmentorBase.augment_impl(Rotate90()))) <: Expr
