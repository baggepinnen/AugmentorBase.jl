@testset "Resize" begin
    @test (Resize <: AugmentorBase.AffineOperation) == false
    @test typeof(@inferred(Resize())) <: Resize <: AugmentorBase.ImageOperation
    @testset "constructor" begin
        ...
        @test str_show(op) == "AugmentorBase.Resize{1}((10,))"
        ...
        @test op.size == (20,30)
        @test str_show(op) == "AugmentorBase.Resize{2}((20, 30))"
        ...
        @test str_show(op) == "AugmentorBase.Resize{3}((20, 30, 40))"
        @test str_showconst(op) == "Resize(20, 30, 40)"
        @test str_showcompact(op) == "Resize to (20, 30, 40)"
    end
    @testset "eager" begin
        @test_throws MethodError AugmentorBase.applyeager(Resize(10,10), nothing)
        @test AugmentorBase.supports_eager(Resize) === true
    ...
    @testset "affine" begin
        @test AugmentorBase.supports_affine(Resize) === false
    end
    @testset "affineview" begin
        @test AugmentorBase.supports_affineview(Resize) === true
        @test_throws MethodError AugmentorBase.applyaffineview(Resize(10,10), nothing)
        @test @inferred(AugmentorBase.toaffinemap(Resize(4,9), rect)) ≈ AffineMap([2. 0.; 0. 3], [-0.5,-1.0])
        for h in (1,2,3,4,5,9), w in (1,2,3,4,5,9)
            wv = @inferred AugmentorBase.applyaffineview(Resize(h,w), AugmentorBase.prepareaffine(square))
        ...
        for h in (1,2,3,4,5,9), w in (1,2,3,4,5,9) # bigger show drift
            wv = @inferred AugmentorBase.applyaffineview(Resize(h,w), AugmentorBase.prepareaffine(checkers))
        ...
        for h in (3,10,29,30,64), w in (3,10,29,30,64)
            ...
            wv = @inferred AugmentorBase.applyaffineview(Resize(h,w), AugmentorBase.prepareaffine(camera))
    ...
    @testset "lazy" begin
        @test AugmentorBase.supports_lazy(Resize) === true
        @testset "single image" begin
            wv = @inferred AugmentorBase.applylazy(Resize(2,3), square)
            @test eltype(wv) == eltype(square)
            @test typeof(wv) <: SubArray
            @test typeof(wv.indices) <: Tuple{Vararg{IdentityRange}}
            @test typeof(parent(wv)) <: InvWarpedView
        ...
        @testset "multiple images" begin
            wv1, wv2 = @inferred AugmentorBase.applylazy(Resize(2,3), (square, square2))
    ...
    @testset "view" begin
        @test AugmentorBase.supports_view(Resize) === false
    end
    @testset "stepview" begin
        @test AugmentorBase.supports_stepview(Resize) === false
    end
    @testset "permute" begin
        @test AugmentorBase.supports_permute(Resize) === false
    end
end
